...
class M_tulisan extends CI_Model{
	...
	function get_berita_slider(){
		...
		$lowongan=$this->db->query("SELECT * 
			FROM lowongan 
			JOIN departement ON lowongan.id_departement = departement.id_departement 
			JOIN jabatan ON lowongan.id_jabatan = jabatan.id_jabatan 
			WHERE status = '1';");
		return $hsl;
	}
